...
def prompt_for(item):
    if "repeat" not in item:
        return item["prompt"]
    block = (
        " Context detail: The application uses HTTP services, a relational database, "
        "structured logging, retries, caching, authentication, configuration, and "
        "background processing. Evaluate these details as part of the stated task."
    )
    ...
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
            ...
            return {
                ...
                "route": headers.get("X-TinyRouter-Route", ""),
                "source": headers.get("X-TinyRouter-Source", ""),
                "l1_latency_ms": float(headers.get("X-TinyRouter-L1-Latency-Ms", 0)),
                "escalation": headers.get("X-TinyRouter-Escalation", "false") == "true",
            }
...
def summarize(rows):
    ...
    for category, items in groups.items():
        ...
        escalated = sum(x["router_escalation"] for x in router)
        expected = EXPECTED[category]
        correct = sum(x["router_route"] == expected for x in router)
        summary.append({
            "category": category,
            "count": len(items),
            "expected_route": expected,
            "route_accuracy": correct / len(router) if router else 0,
            ...
            "l1_avg_ms": statistics.mean(x["router_l1_ms"] for x in router) if router else 0,
            "l1_handled": l1_handled,
            "l1_escalated": escalated,
            "l2_calls": l2_calls,
    ...
    for repeat in range(args.repeats):
        for index, item in enumerate(dataset, 1):
            ...
            rows.append({
                ...
                "router_ok": routed["ok"],
                "router_ms": routed["latency_ms"],
                "router_route": routed.get("route", ""),
                "router_source": routed.get("source", ""),
                "router_l1_ms": routed.get("l1_latency_ms", 0),
                "router_escalation": routed.get("escalation", False),
                "router_input_tokens": routed.get("input_tokens", 0),
                "router_output_tokens": routed.get("output_tokens", 0),
